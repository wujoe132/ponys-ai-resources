# Han So-yeon character SDK record

[Han So-yeon on Ponys.ai](https://ponys.ai/characters/han-so-yeon) is the canonical public page
for the `han-so-yeon` record in the publisher resource SDK.

## Record fields

| Field | Value |
|---|---|
| Slug | `han-so-yeon` |
| Locale | `en` |
| Market | `GLOBAL` |
| Canonical URL | [https://ponys.ai/characters/han-so-yeon](https://ponys.ai/characters/han-so-yeon) |

## Integration use

Use this stable record in feed readers, publisher widgets, API examples, and
link-health checks. Fetch changing descriptions and media from the public feed
rather than caching them in the identity record. This official data card does
not contain private conversations or user-created content.
